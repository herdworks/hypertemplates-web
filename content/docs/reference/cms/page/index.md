...
A page is a directory containing a page file in Markdown (`index.md`), YAML (`index.yaml`), JSON (`index.json`) format, or HTML formats.

### Page files
--------------

Pages are directories with one of the following page files: 
...
**NEW:** page files with `page.*` filenames (`page.md`, `page.yaml`, `page.json`, and `page.html`), are supported in `hyperctl` 0.18.0 and later.
...
Pages created with HTML [page files] are called "static pages".
Static pages are detected as pages during HyperTemplates [builds] and appear in [`hyperctl cms page ls`] output, but they are copied directly into build outputs without any additional processing (i.e. no templating).

Static pages support a limited subset of [page properties](#properties), with the following sources: 

* [**page.title**](#page-title): the HTML `<title>` element inner text
* [**page.description**](#page-description): the HTML `<meta name='description'>` element `content` attribute
* [**page.created_at**](#page-created_at): the HTML `<meta name='created_at'>` element `content` attribute
* [**page.updated_at**](#page-updated_at): the HTML `<meta name='updated_at'>` element `content` attribute

<doc-quote ht-block info>

Page files in HTML format (`index.html` and `page.html`) are treated as "static" pages.

</doc-quote>

<doc-quote ht-block new>

**NEW:** "static" HTML pages (`index.html` and `page.html`), and `page.*` files (`page.md`, `page.yaml`, and `page.json`), are supported in `hyperctl` 0.18.0 and later.
...
: The canonical URL for the page.
  ...
  The `page.canonical_url` property is the canonical URL for the page.

  **Example**

  ```yaml
  canonical_url: "https://hypertemplates.net/blog/introducing-hypertemplates/"
  ```

  <doc-quote ht-block notice>
  **NOTE:** `page.canonical_url` is a computed property.
  It is generated by HyperTemplates and made available for templating purposes only.
  If this value is present in a page index file, it will be overwritten.
  </doc-quote>
...
: The page wordcount.
  ...
  <doc-quote ht-block notice>
  **NOTE:** `page.wordcount` is a computed property.
  It is generated by HyperTemplates and made available for templating purposes only.
  If this value is present in a page index file, it will be overwritten.
  </doc-quote>
...
: A list of pages matching one or more feed filters (`page.feed.tags` or `page.feed.paths`).
  ...
  **Example**
  ...
  <code-snippet ht-block filename='theme/fragments/feed.html' highlight='2-9'>
  ...
  In this example layout fragment we've configured a feed page that will display the title, date, summary, and a "continue reading" link that links to the matching page.
  Pretty simple!
...
[`site.created_at`]: /docs/reference/cms/website/#site-created_at
