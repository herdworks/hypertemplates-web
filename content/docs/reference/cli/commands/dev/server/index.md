...
title: hyperctl dev server
summary: |
    `hyperctl dev server` reference documentation.
breadcrumb: server
---

## `hyperctl dev server` command
...
The `hyperctl dev server` command performs a full website build and serves the resulting build output over HTTP.

### Usage
---------

```plaintext
 $ hyperctl dev server -h
Run a local HTTP server.

Usage:
        hyperctl dev server [options] [content]
