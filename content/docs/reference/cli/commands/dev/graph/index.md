...
title: hyperctl dev graph
summary: |
    `hyperctl dev graph` reference documentation.
breadcrumb: graph
---
...
```plaintext
$ hyperctl dev graph -h
Fetch entity and page metadata from the HyperGraph™️.

Usage:
        hyperctl dev graph <resource> [options] <href>
...
```plaintext
$ hyperctl dev graph document https://www.youtube.com/watch?v=dQw4w9WgXcQ
{
        "author": {
...
**`<resource>`**
: The first argument to `hyperctl dev graph` is the "type" of resource to scrape.

  The supported resources types are: `entity`, or `document`.

**`<href>`**
: The second argument to `hyperctl dev graph` is the URL of the resource to scrape.
