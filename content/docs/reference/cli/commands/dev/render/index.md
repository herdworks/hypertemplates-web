...
title: hyperctl dev render
summary: |
    `hyperctl dev render` reference documentation.
breadcrumb: render
---

## `hyperctl dev render` command
...
The `hyperctl dev render` command provides direct access to the underlying HyperTemplates rendering engine for generating HTML files from a layout and some data.

<doc-quote ht-block info>

**What is it good for?** 🎵

When is it appropriate to use the `hyperctl dev render` command instead of [`hyperctl build`] or [`hyperctl generate`]? 
The `hyperctl dev render` command does not apply any of the opinions that are built in to the [HyperTexting CMS].
...
```plaintext
$ hyperctl dev render -h
Render a HyperTemplate

Usage:
        hyperctl dev render [options]
