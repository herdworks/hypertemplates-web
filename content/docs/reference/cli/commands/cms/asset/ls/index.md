...
title: hyperctl cms asset ls
summary: |
    `hyperctl cms asset ls` command reference.
breadcrumb: ls
---

## `hyperctl cms asset ls` command
...
The `hyperctl cms asset ls` command reads website configuration and lists all assets in the configured theme, website, and content subdirectories.
...
The `hyperctl cms asset ls` command is a helpful utility for listing assets.
...
But we have observed build times for @hypertemplates.net consistently under 100ms, so let's use `hyperctl cms asset ls` to determine what all these 69 assets files are.

```plaintext
$ hyperctl cms asset ls
No  Source                                                    Size      Path
--  ------                                                    ----      ----
...
We should restore the large download assets so that our builds are complete again, but this little exercise illustrates how `hyperctl cms asset ls` is a help utility for asset troubleshooting.
