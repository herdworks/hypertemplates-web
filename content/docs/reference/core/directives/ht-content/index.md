...
title: ht-content directive
summary: |
    `ht-content` directive reference documentation
breadcrumb: ht-content
---

## `ht-content` directive reference

<auto-toc selectors='h3,h4,h5,h6'></auto-toc>

### Overview 
------------
The `ht-content` directive inserts text or HTML content from [template data] into the target HTML element.
...
The `ht-content` directive can be used with any HTML element that can have child nodes.
The `ht-content` directive should not be used with [void elements].
...
The `ht-content` directive provides content templating instructions, expressed as a single `format:value` pair, where `value` is a comma-separated list of dot-notation style references to one or more [template data properties].
...
The `ht-content` directive will only insert content in the target element if the specified [template data property] is found.
In cases where no value is found, HyperTemplates ignores the directive.
...
The `ht-content` directive can optionally specify the format of the content to be inserted into the layout.
