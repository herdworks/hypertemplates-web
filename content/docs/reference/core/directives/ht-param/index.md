...
title: ht-param directive
summary: |
    `ht-param` directive reference documentation
breadcrumb: ht-param
---

## `ht-param` directive reference
...
The `ht-param` directive _replaces_ the target element with text or HTML content from [template data].

### Example
-----------

This example shows the `ht-param` directive being used to template a `<p>` element.
...
```javascript
{
    page: {
        title: "ht-param",
        description: "ht-param directive reference"
    }
}
```
...
The `ht-param` directive can be used with any HTML element.
The `ht-param` directive is best used with [void elements], simply because they do not require closing tags and thus improve template brevity.
...
Because we prefer to use the `ht-param` directive with [Void elements], and the only void element that isn't actively in use is the `<param>` element.
And we like how it looks in our templates. 🤓
But the most important reason is because HyperTemplates will **always** remove or replace elements with `ht-param` directives, so we'll never see them in rendered HTML.

</doc-quote>

<doc-quote ht-block danger>

**NOTE:** although the `ht-param` directive can be added to any HTML element, `ht-param` elements cannot be nested inside of elements that only support [Text nodes], such as the [`<title>`] element.
...
The `ht-param` directive provides content templating instructions, expressed as a single `format:value` pair, where `value` is a comma-separated list of dot-notation style references to one or more [template data properties].
...
The `ht-param` directive will only insert content if the specified [template data property] is found.
...
The `ht-param` directive can optionally specify the format of the data to be inserted into the layout.
