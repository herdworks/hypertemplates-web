...
title: ht-query directive
summary: |
    `ht-query` directive reference documentation
breadcrumb: ht-query
---

## `ht-query` directive reference
...
The `ht-query` directive _annotates_ URLs in HTML attributes with query parameters.

<doc-quote ht-block caution>
**NEW!** The `ht-query` directive is available in `hyperctl` version 0.16.0 and later.
</doc-quote>

### Example
-----------

The `ht-query` directive is used to annotate URLs with query parameters.
...
In this example the `ht-query` directive is used to append a `?version=` query parameter to our CSS and JavaScript files.
...
Nice!
With this example `ht-query` directive we can version our CSS and JavaScript files by changing a single property.
...
The `ht-query` directive can be used with any HTML element.
...
The `ht-query` directive is expressed as a semicolon-separated list of `attribute?param:value` triplets.
